...
def _get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Accepts a python expression that will be evaluated as a list, which will be used to gather model runs.",
    )
...
def _split_data_across_runs(data, start=1, end=None, gather=None):
    """
    ...
    model_run_splits = [i + 1 for i, item in enumerate(data) if item.get("name") == "model_run"]
    ...
    data_splits = [data[model_run_splits[i] : model_run_splits[i + 1]] for i in range(total_num_runs)]

    if gather is not None:
        gather_list = list(eval(gather))
        gather_list = [i for i in gather_list if 0 <= i < total_num_runs]
        print(f"Gathering model runs according to the specified '{gather}', ignoring --start and --end.")
        num_runs = len(gather_list)
        runs = f"{gather_list}"
    else:
        ...
        runs = f"{start}-{end - 1}"
        gather_list = list(range(start, end))

    print(f"Analyzing {num_runs} model run(s): {runs}.")

    # Gather model runs and flatten the list
    gathered_data = [item for i in gather_list for item in data_splits[i]]
    return gathered_data, len(gather_list)
...
def main():
    ...
    data, num_runs = _split_data_across_runs(data, args.start, args.end, args.gather)
